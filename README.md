...
- [x] 普通、路障和铁桶僵尸的行走、受阻、啃咬和死亡
- [x] 直线逻辑豌豆和 Rapier 抛射物理豌豆
- [x] 阳光拾取、资源消耗和植物卡片冷却
- [x] RON 配置的 10 波 34 只僵尸、胜利、失败和重开
- [x] HUD、组合色块建模和 Rapier 碰撞体调试渲染
- [x] 18 项自动化测试及零警告 Clippy 检查
- [ ] 原创精灵、动画、音效、粒子和命中反馈
...
- 主草坪下方空一整行后是 9 格豌豆射手专用行；该行豌豆射出后传送到主草坪最左格。
- 向日葵只能种在空中 6 格；专用行只能种普通豌豆射手；僵尸只在主草坪生成和寻找目标。
...
内容目录记录了全部僵尸的可啃食有效血量；当前波次使用普通、路障和铁桶僵尸。
