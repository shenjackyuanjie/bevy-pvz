...
- [x] 向日葵、双头向日葵、四种豌豆射手、坚果和火炬树桩
- [x] 普通、路障、铁桶、橄榄球、读报、铁门和冰车僵尸
- [x] 直线逻辑豌豆和 Rapier 抛射物理豌豆
- [x] 阳光拾取、资源消耗和植物卡片冷却
- [x] RON 配置关卡、固定植物卡片、胜利、失败和重开
- [x] HUD、组合色块建模和 Rapier 碰撞体调试渲染
- [x] 43 项自动化测试及零警告 Clippy 检查
- [ ] 原创精灵、动画、音效、粒子和命中反馈
...
| 空格 | 暂停/继续游戏；暂停时显示全屏灰色遮罩 |
| `R` | 随时重开当前关卡；胜负画面中开始新一局 |

HUD 显示当前阳光、波次、击杀数、关卡时间、弹丸/僵尸数量、植物价格与冷却，并用进度条显示未消灭僵尸占全关的比例。

## 玩法实现

### 草坪与种植

- 默认关卡从 `assets/levels/level_01.ron` 加载，也可以通过 `--level` / `-l` 指定其他 RON；植物卡片列表在代码中固定，不写入关卡 RON。
- 主草坪是一条 9 格道路；第 1 列和末尾 2 列上方各有 2 格空中草坪，共 6 格。
- row `0` 只允许坚果或火炬树桩；row `-1` 留空；row `-2` 只允许四种豌豆射手或火炬树桩。
- 底排豌豆先向左飞，到草坪左边界后传送到 row `0` 左端，再改为向右飞行。
- 向日葵只能种在空中 6 格；双头向日葵只能覆盖在普通向日葵上；僵尸只在 row `0` 生成和寻找目标。
...
| `ui.rs` | HUD、僵尸进度条、卡片与操作提示 |
| `ui/results.rs` | 胜负画面和结果清理 |
