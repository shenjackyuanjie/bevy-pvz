...
- [x] 最下面一条 12 格道路、6 个空中向日葵格和拖拽种植
- [x] 向日葵、豌豆射手、多发射手、机枪射手和坚果
- [x] 普通僵尸的行走、受阻、啃咬和死亡
- [x] 直线逻辑豌豆和 Rapier 抛射物理豌豆
- [x] 阳光拾取、资源消耗和植物卡片冷却
- [x] RON 配置的 8 波 32 只僵尸、胜利、失败和重开
...
- 底层是一条 12 格道路；第 1 列和末尾 2 列上方各有 2 格空中草坪，共 6 格。
- 向日葵只能种在空中 6 格，其他植物只能种在底层；僵尸只在底层生成和寻找目标。
- `LawnLayout` 集中管理列数、单元格尺寸、道路高度、空中格和世界原点。
- 世界坐标和 `GridCell` 的换算是无 ECS 的纯函数。
- `CellOccupancy` 保证一个格子只能存在一株植物。
- 从横向植物卡片栏按下鼠标会生成跟随光标的植物预览，松开到合法空格后才发出种植请求。
...
- 地板顶面与草坪底边保留 50 像素垂直间距并带少量摩擦；未命中时不会超时或越界销毁，直到关卡重置。
...
| `lawn.rs` | 底层道路与空中草坪布局、二维格子坐标、占用和占位绘制 |
